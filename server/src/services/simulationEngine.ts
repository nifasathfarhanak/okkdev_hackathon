...
export interface SimulationAssumptions {
  budgetLimit: number;
  deadlineDays: number;
  supplierDelayProb?: number; // default 15%
  priceVolatility?: number; // default 8%
  logisticsReliability?: number; // default 92%
  negotiationSuccessRate?: number; // default 75%
  demandPressure?: number; // default 20%
}
...
export interface SimulationResultMetrics {
  ...
  planName: string;
  iterations: number;
  simulatedSuccessPct: number;
  simulatedDelayPct: number;
  simulatedFailurePct: number;
  medianCost: number;
  costP10: number;
  costP50: number;
  costP90: number;
  deliveryDaysP10: number;
  deliveryDaysP50: number;
  deliveryDaysP90: number;
  downsideRiskScore: number;
  complexityScore: number;
  constraintViolationsCount: number;
  scenarioDistribution: {
    baseline: { runs: number; success: number; delay: number; failure: number };
    logisticsDelay: { runs: number; success: number; delay: number; failure: number };
    priceSpike: { runs: number; success: number; delay: number; failure: number };
    capacityShortage: { runs: number; success: number; delay: number; failure: number };
    combinedStress: { runs: number; success: number; delay: number; failure: number };
  };
}

export interface DecisionWeights {
  costWeight: number; // e.g. 0.30
  speedWeight: number; // e.g. 0.30
  reliabilityWeight: number; // e.g. 0.25
  complexityWeight: number; // e.g. 0.15
}
...
export class SimulationEngine {
  ...
  static simulatePlan(
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      let scenarioType: 'baseline' | 'logisticsDelay' | 'priceSpike' | 'capacityShortage' | 'combinedStress' = 'baseline';
    ...
    const getP = (arr: number[], pct: number) => arr[Math.floor(arr.length * pct)];
  ...
  static comparePlans(
    plans: PlanData[],
    results: SimulationResultMetrics[],
    weights: DecisionWeights = { costWeight: 0.30, speedWeight: 0.30, reliabilityWeight: 0.25, complexityWeight: 0.15 }
  ): {
    rankedPlans: Array<{
      planKey: string;
      planName: string;
