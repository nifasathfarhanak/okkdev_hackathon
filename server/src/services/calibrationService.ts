...
export interface CalibrationResult {
  intentId: string;
  predictedCostMedian: number;
  predictedDeliveryP50: number;
  predictedSuccessPct: number;
  actualCost: number;
  actualDeliveryDays: number;
  costVariance: number;
  deliveryVariance: number;
  accuracyTier: 'HIGH_ACCURACY' | 'CLOSE_PREDICTION' | 'MILD_UNDERESTIMATE' | 'SIGNIFICANT_DEVIATION';
...
export class CalibrationService {
  ...
  static async recordAndCalibrate(params: {
    ...
    let accuracyTier: 'HIGH_ACCURACY' | 'CLOSE_PREDICTION' | 'MILD_UNDERESTIMATE' | 'SIGNIFICANT_DEVIATION' = 'HIGH_ACCURACY';
    let feedbackNote = '';
