...
exports.mirrorRouter = void 0;
const express_1 = require("express");
const prisma_1 = require("../db/prisma");
const simulationEngine_1 = require("../services/simulationEngine");
const counterAgentService_1 = require("../services/counterAgentService");
const whatIfService_1 = require("../services/whatIfService");
const calibrationService_1 = require("../services/calibrationService");
const sseService_1 = require("../services/sseService");
exports.mirrorRouter = (0, express_1.Router)();
/**
 * POST /api/mirror/generate-plans
 * Generates 4 competing action paths for an intent.
 */
exports.mirrorRouter.post('/generate-plans', async (req, res) => {
    try {
        const { title, description, budget, deadlineDays, quantity } = req.body;
        if (!title) {
            return res.status(400).json({ error: 'Intent title is required.' });
        }
        const plans = simulationEngine_1.SimulationEngine.generatePlans({
            title,
 ...
 */
exports.mirrorRouter.post('/simulate', async (req, res) => {
    try {
        const { intent: intentInput, iterations = 4000, seed = 4289, assumptions: customAssumptions = {} } = req.body;
        if (!intentInput || !intentInput.title) {
            return res.status(400).json({ error: 'Intent object with title is required.' });
        }
        sseService_1.sseService.broadcast({ type: 'SIMULATION_STATUS', message: 'Creating intent & generating competing plans...', progress: 10 });
        ...
        // 1. Save Intent in DB
        const intent = await prisma_1.prisma.intent.create({
            data: {
        ...
        // 2. Generate Plans
        const plansData = simulationEngine_1.SimulationEngine.generatePlans({
            title: intent.title,
        ...
        for (const p of plansData) {
            const sp = await prisma_1.prisma.plan.create({
                data: {
                    intentId: intent.id,
                    planKey: p.planKey,
                    name: p.name,
        ...
        sseService_1.sseService.broadcast({ type: 'SIMULATION_STATUS', message: `Running ${iterations.toLocaleString()} Monte Carlo scenario simulations...`, progress: 40 });
        ...
        for (const p of savedPlans) {
            const resMetrics = simulationEngine_1.SimulationEngine.simulatePlan(p, assumptions, Math.floor(iterations / 4), seed);
            simResults.push(resMetrics);
        }
        const comparison = simulationEngine_1.SimulationEngine.comparePlans(savedPlans, simResults);
        // 4. Generate Counter-Agent Challenges
        sseService_1.sseService.broadcast({ type: 'SIMULATION_STATUS', message: 'Deploying Adversarial Counter-Agent for stress testing...', progress: 75 });
        const challenges = counterAgentService_1.CounterAgentService.generateChallenges({ title: intent.title, budget, deadlineDays }, savedPlans, simResults);
        // 5. Save Simulation Record
        const simCode = `AM-${Math.floor(10000 + Math.random() * 90000)}`;
        const simulation = await prisma_1.prisma.simulation.create({
            data: {
                intentId: intent.id,
        ...
        await prisma_1.prisma.intent.update({
            where: { id: intent.id },
        ...
        sseService_1.sseService.broadcast({ type: 'SIMULATION_STATUS', message: 'Simulation complete. Decision Brief ready.', progress: 100 });
        return res.json({
            success: true,
            simulationId: simulation.id,
 ...
 */
exports.mirrorRouter.get('/simulations', async (req, res) => {
    try {
        const simulations = await prisma_1.prisma.simulation.findMany({
            include: {
                intent: {
                    include: {
                        plans: true,
 ...
 */
exports.mirrorRouter.get('/simulation/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const simulation = await prisma_1.prisma.simulation.findFirst({
            where: {
                OR: [{ id: id }, { simulationCode: id }]
            },
            include: {
                intent: {
                    include: {
        ...
        const plansData = sim.intent.plans.map((p) => ({
            id: p.id,
            planKey: p.planKey,
            name: p.name,
            description: p.description,
        ...
        const metrics = plansData.map(p => simulationEngine_1.SimulationEngine.simulatePlan(p, assumptions, Math.floor(simulation.iterations / 4), simulation.seed));
        const comparison = simulationEngine_1.SimulationEngine.comparePlans(plansData, metrics);
        return res.json({
            success: true,
            simulation,
            plans: plansData,
 ...
 */
exports.mirrorRouter.post('/simulation/:id/what-if', async (req, res) => {
    try {
        const { id } = req.params;
        const { modifiedAssumptions } = req.body;
        const simulation = await prisma_1.prisma.simulation.findFirst({
            where: { OR: [{ id: id }, { simulationCode: id }] },
        ...
        const plansData = sim2.intent.plans.map((p) => ({
            id: p.id,
            planKey: p.planKey,
            name: p.name,
            description: p.description,
        ...
        const deltaResult = whatIfService_1.WhatIfService.recalculate(plansData, origAssumptions, newAssumptions, Math.floor(simulation.iterations / 4), simulation.seed);
 ...
 * Human Approval Gate
 */
exports.mirrorRouter.post('/simulation/:id/approve', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const sim3 = simulation;
        const planKey = selectedPlanKey || sim3.preferredPlanKey || 'PLAN_B';
        const chosenPlan = sim3.intent.plans.find((p) => p.planKey === planKey) || sim3.intent.plans[0];
 ...
 */
exports.mirrorRouter.post('/simulation/:id/execute', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const sim4 = simulation;
        const plan = sim4.intent.plans.find((p) => p.planKey === planKey) || sim4.intent.plans[0];
 ...
 */
exports.mirrorRouter.post('/simulation/:id/outcome', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const sim5 = simulation;
        const calibrationResult = await calibrationService_1.CalibrationService.recordAndCalibrate({
 ...
 */
exports.mirrorRouter.get('/calibration', async (req, res) => {
    try {
        const calibrations = await prisma_1.prisma.calibration.findMany({
            include: {
                intent: {
                    include: {
                        actualOutcome: true,
