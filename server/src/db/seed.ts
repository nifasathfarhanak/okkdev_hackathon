...
export async function seed() {
  ...
  try {
    ...
    await prisma.a2ASession.deleteMany();
    await prisma.integration.deleteMany();
    await prisma.activityLog.deleteMany();
    await prisma.calibration.deleteMany();
    await prisma.actualOutcome.deleteMany();
    await prisma.execution.deleteMany();
    await prisma.approval.deleteMany();
    await prisma.decisionBrief.deleteMany();
    await prisma.counterChallenge.deleteMany();
    await prisma.scenarioOutcome.deleteMany();
  ...
  const planD = await prisma.plan.create({
    data: {
  ...
  const demoSimulation = await prisma.simulation.create({
    data: {
      id: 'sim-demo-00182',
      intentId: demoIntent.id,
  ...
  for (let i = 0; i < historicalItems.length; i++) {
    ...
    const hIntent = await prisma.intent.create({
      data: {
        userId: primaryUser.id,
        title: h.title,
    ...
    const hPlan = await prisma.plan.create({
      data: {
        intentId: hIntent.id,
        planKey: 'PLAN_A',
    ...
    // Actual Outcome
    await prisma.actualOutcome.create({
      data: {
        intentId: hIntent.id,
