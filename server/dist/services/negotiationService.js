...
class NegotiationService {
    async executeNegotiation(dealId, buyerAgentId, supplierAgentId, targetPrice, targetDeliveryDays) {
        const deal = await prisma_1.prisma.deal.findUnique({
            where: { id: dealId },
            include: { requirement: true }
        });
        const supplier = await prisma_1.prisma.agent.findUnique({
        ...
        let negotiation = await prisma_1.prisma.negotiation.findFirst({
            where: { dealId, sellerAgentId: supplierAgentId }
        });
        if (!negotiation) {
            negotiation = await prisma_1.prisma.negotiation.create({
                data: {
                    dealId,
                    buyerAgentId,
        ...
        for (let round = 1; round <= 3 && !isConcluded; round++) {
            ...
            await prisma_1.prisma.negotiationMessage.create({
                data: {
            ...
            await prisma_1.prisma.negotiation.update({
            ...
            sseService_1.sseService.broadcast('negotiation.message', {
                dealId,
                negotiationId: negotiation.id,
                round,
...
exports.negotiationService = new NegotiationService();
